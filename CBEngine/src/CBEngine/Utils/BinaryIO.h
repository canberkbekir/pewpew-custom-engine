...
namespace CB
{
	...
	public:
		...
		FileStream(const char* path,const char* mode) { Open(path, mode); }
		...
		FileStream(FileStream&& other) noexcept
			: m_File(other.m_File) { other.m_File = nullptr; }
		...
		void Flush() override { if (m_File) std::fflush(m_File); }
	...
	public:
		...
		{
			if constexpr (std::is_trivially_copyable_v<T>) { WriteBytes(&v, sizeof(T)); }
			else {
	...
	public:
		...
		void Read(T& v)
		{
			if constexpr (std::is_trivially_copyable_v<T>) { ReadBytes(&v, sizeof(T)); }
			else {
				// Non-trivial types must provide:
