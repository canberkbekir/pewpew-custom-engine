...
namespace CB
{
	...
	UUID AssetManager::ImportAsset(const std::filesystem::path& filePath)
	{
		...
		CB_CORE_INFO("Imported asset: {0} (UUID: {1})", relativePath.string(), (uint64_t)uuid);
	...
	void AssetManager::ReloadAsset(UUID uuid)
	{
		...
		if (asset)
		{
			if (asset->Reload())
			{
				CB_CORE_INFO("Reloaded asset: {0}", (uint64_t)uuid);
			...
			{
				CB_CORE_ERROR("Failed to reload asset: {0}", (uint64_t)uuid);
			}
		}
	}
	...
	Ref<Asset> AssetManager::LoadAssetInternal(const AssetMetadata& metadata)
	{
		...
		if (asset)
		{
			...
			CB_CORE_INFO("Loaded asset: {0} (UUID: {1})", fullPath.string(), (uint64_t)metadata.Handle);
		}
