...
namespace CB
{
	...
	void AssetManager::RegisterLoader(AssetType type,AssetLoaderFn loader) { s_Loaders[type] = std::move(loader); }

	void AssetManager::RegisterExtension(const std::string& ext,AssetType type) { s_ExtensionMap[ext] = type; }

	AssetType AssetManager::AllocateCustomType() { return static_cast<AssetType>(s_NextCustomType++); }
	...
	void AssetManager::ReloadAsset(UUID uuid)
	{
		...
		if (asset) {
			if (asset->Reload()) {
				...
				for (UUID dependent : dependents) { ReloadAsset(dependent); }
			}
			else { CB_CORE_ERROR("Failed to reload asset: {0}", static_cast<uint64_t>(uuid)); }
		}
	}
	...
	void AssetManager::RenameAsset(UUID uuid,const String& newName)
	{
		...
		if (exists(oldMetaPath)) { std::filesystem::rename(oldMetaPath, newMetaPath); }
	...
	void AssetManager::DeleteAsset(UUID uuid)
	{
		...
		// Delete files
		if (exists(fullPath)) { std::filesystem::remove(fullPath); }

		std::filesystem::path metaPath = GetMetaFilePath(fullPath);
		if (exists(metaPath)) { std::filesystem::remove(metaPath); }
	...
	void AssetManager::LoadMetaFile(const std::filesystem::path& metaPath)
	{
		...
		try { uuidValue = std::stoull(uuidStr); }
	...
	void AssetManager::CreateMetaFile(const std::filesystem::path& assetPath,UUID uuid)
	{
		...
		if (file.fail()) { CB_CORE_ERROR("Failed to write meta file: {0}", metaPath.string()); }
	}
	...
	Ref<Asset> AssetManager::LoadAssetInternal(const AssetMetadata& metadata)
	{
		...
		if (loaderIt != s_Loaders.end()) { asset = loaderIt->second(metadata); }
		else if (metadata.Type == AssetType::Scene) { CB_CORE_WARN("Scene loading requires ECS implementation"); }
		else { CB_CORE_ERROR("No loader registered for asset type: {0}", static_cast<int>(metadata.Type)); }
