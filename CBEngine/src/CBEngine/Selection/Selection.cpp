...
namespace CB
{
	...
	void Selection::Select(const Selectable& item)
	{
		...
		if (item.IsValid()) { s_Selection.push_back(item); }
	...
	void Selection::AddSelectionChangedCallback(SelectionChangedCallback callback) { s_Callbacks.push_back(callback); }

	void Selection::RemoveAllCallbacks() { s_Callbacks.clear(); }

	void Selection::NotifySelectionChanged() { for (const auto& callback : s_Callbacks) { callback(s_Selection); } }
}