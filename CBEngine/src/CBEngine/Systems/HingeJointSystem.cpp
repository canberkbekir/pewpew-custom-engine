...
namespace CB
{
		...
		for (auto e : view) {
			...
			if (hj.BreakForce > 0.0f || hj.BreakTorque > 0.0f) {
				...
				if ((hj.BreakForce > 0.0f && linearForce > hj.BreakForce) ||
					(hj.BreakTorque > 0.0f && angularTorque > hj.BreakTorque)) { shouldBreak = true; }
			}
	...
	void HingeJointSystem::InitConstraint(Scene* scene,entt::entity e)
	{
		...
		auto worldAnchor1 = Vector3(world1 * glm::vec4(hj.Anchor, 1.0f));
		...
		if (connectedEntity && connectedEntity.HasComponent<RigidBodyComponent>()) {
			...
			constraint = settingsPtr->Create(lock1.GetBody(), lock2.GetBody());
		}
		else {
			JPH::BodyLockWrite lock1(lockInterface, rb.RuntimeBodyID);
			if (!lock1.Succeeded()) {
			...
			constraint = settingsPtr->Create(lock1.GetBody(), JPH::Body::sFixedToWorld);
		}
