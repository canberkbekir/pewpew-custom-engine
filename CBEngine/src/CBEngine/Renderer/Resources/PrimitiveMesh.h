	...
	private:
		...
		static std::pair<std::vector<Vertex>, std::vector<uint32_t>> GenerateSphere()
		{
			...
			for (int stack = 0; stack <= stacks; stack++) {
				float phi = PI * static_cast<float>(stack) / static_cast<float>(stacks); // 0 (top) -> PI (bottom)
		...
		static std::pair<std::vector<Vertex>, std::vector<uint32_t>> GenerateCylinder()
		{
			...
			uint32_t topRingBase = static_cast<uint32_t>(vertices.size());
			for (int i = 0; i <= segments; i++) {
				float theta = 2.0f * PI * static_cast<float>(i) / static_cast<float>(segments);
				float x = radius * std::cos(theta);
				float z = radius * std::sin(theta);
			...
			uint32_t botRingBase = static_cast<uint32_t>(vertices.size());
			for (int i = 0; i <= segments; i++) {
				float theta = 2.0f * PI * static_cast<float>(i) / static_cast<float>(segments);
				float x = radius * std::cos(theta);
				float z = radius * std::sin(theta);
		...
		static std::pair<std::vector<Vertex>, std::vector<uint32_t>> GenerateCapsule()
		{
			...
			constexpr int hemiStacks = 8;
			constexpr int slices = 32;
			constexpr float radius = 0.5f;
			constexpr float cylHalfH = 0.5f;
			constexpr float PI = glm::pi<float>();
			...
			auto addRing = [&](float phi,float yOffset)
			{
				...
				for (int slice = 0; slice <= slices; slice++) {
					...
					v.Position = {r * cosTheta, y, r * sinTheta};
					v.Normal = {sinPhi * cosTheta, cosPhi, sinPhi * sinTheta};
					v.TexCoords = {static_cast<float>(slice) / static_cast<float>(slices), 0}; // v set below
					v.Tangent = {-sinTheta, 0, cosTheta};
					v.Bitangent = glm::cross(v.Normal, v.Tangent);
					vertices.push_back(v);
				}
			...
			for (int s = 0; s <= hemiStacks; s++) {
				float phi = PI * 0.5f * static_cast<float>(s) / static_cast<float>(hemiStacks);
				addRing(phi, cylHalfH);
			}
			// Bottom hemisphere: phi PI/2 (equator) → PI (pole), yOffset = -cylHalfH
			for (int s = 0; s <= hemiStacks; s++) {
				float phi = PI * 0.5f + PI * 0.5f * static_cast<float>(s) / static_cast<float>(hemiStacks);
				addRing(phi, -cylHalfH);
			}
