...
namespace CB
{
	...
	{
		scene->SetEventCallback([](Event& e) { Application::Get().OnEvent(e); });
	}
	...
	bool SceneManager::SaveSceneAs(const String& filepath) { return SaveScene(filepath); }
	...
	void SceneManager::AddSceneChangedCallback(SceneChangedCallback callback) { s_Callbacks.push_back(callback); }

	void SceneManager::ClearCallbacks() { s_Callbacks.clear(); }

	void SceneManager::NotifySceneChanged() { for (auto& callback : s_Callbacks) { callback(s_ActiveScene); } }
}