...
namespace CB
{
    ...
    struct HingeChainComponent
    {
        ...
        void Serialize(YAML::Emitter& out) const
        {
            ...
            out << YAML::Key << "MeshUUID"       << YAML::Value << static_cast<uint64_t>(MeshUUID);
            out << YAML::Key << "MaterialUUID"   << YAML::Value << static_cast<uint64_t>(MaterialUUID);
            out << YAML::Key << "LinkScale"      << YAML::Value << LinkScale;
            out << YAML::Key << "BlueprintUUID"  << YAML::Value << static_cast<uint64_t>(BlueprintUUID);
        ...
        void Deserialize(const YAML::Node& node)
        {
            if (node["LinkType"])
            {
                ...
                {
                    std::string s = node["LinkType"].as<std::string>("");
                    if (!s.empty())
