...
namespace PewPew
{
    ...
    bool Material::LoadFromFile(const String& filePath)
    {
        ...
        if (!file.is_open())
        {
            PEW_CORE_ERROR("Failed to open material file: {0}", filePath);
            return false;
        }
            ...
            else if (key == "albedoMap")
            {
                if (!value.empty())
                {
                    ...
                    if (std::filesystem::exists(texPath))
                        ...
                        PEW_CORE_WARN("Albedo map not found: {0}", texPath.string());
                }
            }
            else if (key == "normalMap")
            {
                if (!value.empty())
                {
                    ...
                    if (std::filesystem::exists(texPath))
                        ...
                        PEW_CORE_WARN("Normal map not found: {0}", texPath.string());
                }
            }
            else if (key == "metallicMap")
            {
                if (!value.empty())
                {
                    ...
                    if (std::filesystem::exists(texPath))
                        ...
                        PEW_CORE_WARN("Metallic map not found: {0}", texPath.string());
                }
            }
            else if (key == "roughnessMap")
            {
                if (!value.empty())
                {
                    ...
                    if (std::filesystem::exists(texPath))
                        ...
                        PEW_CORE_WARN("Roughness map not found: {0}", texPath.string());
                }
            }
        }

        PEW_CORE_INFO("Loaded material: {0}", filePath);
        return true;
    }
    ...
    bool Material::Save(const String& filePath) const
    {
        ...
        if (!file.is_open())
        {
            PEW_CORE_ERROR("Failed to create material file: {0}", filePath);
            return false;
        }

        file << "# PewPew Material File\n";
        ...
        if (file.fail())
        {
            PEW_CORE_ERROR("Failed to write material file: {0}", filePath);
            return false;
        }

        PEW_CORE_INFO("Saved material: {0}", filePath);
        return true;
    }
    ...
    bool Material::Reload()
    {
        ...
        {
            PEW_CORE_WARN("Cannot reload material: not loaded from file");
            return false;
        }
        ...
        {
            PEW_CORE_INFO("Reloaded material: {0}", m_FilePath);
            return true;
        }

        PEW_CORE_ERROR("Failed to reload material: {0}", m_FilePath);
        return false;
    }
