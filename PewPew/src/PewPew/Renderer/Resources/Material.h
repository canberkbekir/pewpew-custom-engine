...
namespace PewPew
{
	...
	public:
		...
		void SetSmoothAmount(float amount) { m_SmoothAmount = amount; }
		void SetStylizedAmount(float amount) { m_StylizedAmount = amount; }
		void SetShaderMode(int mode) { m_ShaderMode = mode; }
		...
		float GetSmoothAmount() const { return m_SmoothAmount; }
		float GetStylizedAmount() const { return m_StylizedAmount; }
		int GetShaderMode() const { return m_ShaderMode; }
	...
	private:
		...
		float m_SmoothAmount = 1.0f;
		float m_StylizedAmount = 0.0f;
		int m_ShaderMode = 0;      // 0=Normal, 1=Stylized, 2=Voxel, 3=Pixel
		float m_PixelSize = 64.0f;
