...
PewPew::Shader::Shader(const std::string& vertexSrc,const std::string& fragmentSrc)
{
	// Create an empty vertex shader handle
		...
		if (isCompiled == GL_FALSE)
		{
			...
			PEW_CORE_ASSERT(false, "Vertex shader compilation failure!");
			return;
		}
		...
		if (isCompiled == GL_FALSE)
		{
			...
			PEW_CORE_ERROR("{0}", infoLog.data());
			PEW_CORE_ASSERT(false, "Fragment shader compilation failure!");
			return;
		}
...
PewPew::Shader::~Shader()
{
	glDeleteProgram(m_RendererID); 
}
void PewPew::Shader::Bind()
{
