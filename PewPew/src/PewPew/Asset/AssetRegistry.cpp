...
namespace PewPew
{
	...
	void AssetRegistry::Register(const AssetMetadata& metadata)
	{
		...
		PEW_CORE_TRACE("AssetRegistry: Registered {0} -> UUID {1}", normalizedPath, (uint64_t)metadata.Handle);

		// Update reverse dependencies
		for (UUID dependency : metadata.Dependencies)
		{
			m_ReverseDependencies[dependency].push_back(metadata.Handle);
		}
	}
