...
namespace PewPew
{
	...
	void ProfilerPanel::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(PEW_BIND_EVENT_FN(ProfilerPanel::OnKeyPressed));
	}

	bool ProfilerPanel::OnKeyPressed(KeyPressedEvent& e)
	{
		if (e.GetKeyCode() == PEW_KEY_F3 && e.GetRepeatCount() == 0)
		{
			ToggleVisibility();
			return true;
		}
	...
	void ProfilerPanel::RenderScopeTree()
	{
		...
		// Simple flat list with indentation based on timing overlap
		// For a proper tree, you'd need parent/child tracking in the profiler
