...
namespace PewPew {
	...
	void Entity::SetParent(Entity newParent,bool keepWorldPosition)
	{
		...
		if (newParent)
		{
			...
			{
				Mat4 parentInverse = glm::inverse(newParent.GetComponent<TransformComponent>().WorldMatrix);
	...
	void Entity::RemoveParent(bool keepWorldPosition)
	{
		SetParent(Entity{}, keepWorldPosition);
	}
