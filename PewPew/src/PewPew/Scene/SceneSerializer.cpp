...
namespace PewPew
{
	...
	void SceneSerializer::SerializeText(const String& FilePath)
	{
		...
		if (!fout.is_open())
		{
			PEW_CORE_ERROR("Failed to open scene file for writing: {0}", FilePath);
			return;
		}
		...
		if (fout.fail())
		{
			PEW_CORE_ERROR("Failed to write scene file: {0}", FilePath);
			return;
		}

		PEW_CORE_INFO("Serialized scene to: {0}", FilePath);
	}
	...
	void SceneSerializer::DeserializeText(const String& FilePath)
	{
		...
		if (!fin.is_open())
		{
			PEW_CORE_ERROR("Failed to open scene file: {0}", FilePath);
			return;
		}
		...
		{
			PEW_CORE_ERROR("Failed to parse scene file: {0}\n{1}", FilePath, e.what());
			return;
		}

		if (!data["Scene"])
		{
			PEW_CORE_ERROR("Invalid scene file (missing 'Scene' key): {0}", FilePath);
			return;
		}
		...
		PEW_CORE_INFO("Deserialized scene from: {0}", FilePath);
	}
}
