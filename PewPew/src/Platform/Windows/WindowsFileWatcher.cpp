...
namespace PewPew
{
	...
	void FileWatcher::Start()
	{
		...
		{
			PEW_CORE_ERROR("FileWatcher: Failed to open directory: {0}", m_Directory.string());
			return;
		}
		...
		PEW_CORE_INFO("FileWatcher started for: {0}", m_Directory.string());
	}
	...
	void FileWatcher::Stop()
	{
		...
		PEW_CORE_INFO("FileWatcher stopped");
	}
	...
	void FileWatcher::WatchThread()
	{
		...
		while (m_Running)
		{
			...
			if (!success)
			{
				...
				{
					PEW_CORE_ERROR("FileWatcher: ReadDirectoryChangesW failed: {0}", error);
					break;
				}
			}
