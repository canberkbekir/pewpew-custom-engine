...
namespace CB
{
		...
		if (ImGui::BeginDragDropTarget()) {
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM")) {
				...
				if (scene) {
					...
					else if (ext == ".scene") { SceneManager::LoadScene(droppedPath.string()); }
	...
	void ViewportPanel::RenderToolbar()
	{
		...
		if (ImGui::ImageButton(
			"##wireframe",
			(ImTextureID)static_cast<uintptr_t>(m_WireframeButtonIcon->GetRendererID()),
			m_IconSize,
			ImVec2(0, 1),
			ImVec2(1, 0),
			ImVec4(0, 0, 0, 0),
			ImVec4(0.9f, 0.9f, 0.9f, 1.0f))) { m_Wireframe = !m_Wireframe; }
		...
		if (mouseX >= 0 && mouseY >= 0 && mouseX < static_cast<int>(spec.Width) && mouseY < static_cast<int>(spec.
			...
			if (pixelData >= 0) {
				...
				if (scene) {
					...
					auto entityHandle = static_cast<entt::entity>(static_cast<uint32_t>(pixelData));
	...
	void ViewportPanel::RenderGizmo()
	{
		...
		// --- Main entity gizmo ---
		{
			...
			if (ImGuizmo::IsUsing()) {
				...
				if (m_GizmoOperation == ImGuizmo::TRANSLATE) { tc.Position = Vector3(transformMatrix[3]); }
				else if (m_GizmoOperation == ImGuizmo::ROTATE) {
