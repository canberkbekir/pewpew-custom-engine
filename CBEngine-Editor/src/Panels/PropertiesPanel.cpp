...
namespace CB
{
	void PropertiesPanel::OnImGuiRender()
	{
		...
		if (ImGui::Button(m_Locked ? "Locked" : "Lock")) {
			if (m_Locked) { Unlock(); }
			else if (Selection::HasSelection()) { Lock(); }
		}

		if (wasLocked) { ImGui::PopStyleColor(2); }

		if (ImGui::IsItemHovered()) {
			ImGui::SetTooltip(m_Locked
				                  ? "Click to unlock and follow selection"
				                  : "Lock to keep current view while browsing");
		}
		...
		Selectable primary;
		if (m_Locked) { primary = m_LockedSelection; }
		else if (Selection::HasSelection()) { primary = Selection::GetPrimarySelection(); }
		else {
