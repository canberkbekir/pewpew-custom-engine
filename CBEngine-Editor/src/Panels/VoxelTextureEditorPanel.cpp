...
namespace CB
{
	...
	void VoxelTextureEditorPanel::Render2DGridView()
	{
		...
		float baseCellSize = glm::min(avail.x / static_cast<float>(sizeX), avail.y / static_cast<float>(sizeZ));
		...
		auto gridOrigin = ImVec2(cursorPos.x + offsetX, cursorPos.y + offsetY);

		// Invisible button to capture input over the full area
		ImGui::InvisibleButton("##2DGrid", avail,
		...
		if (isHovered) {
			...
			if (scroll != 0.0f) {
				...
				m_GridPanOffset.x = mx - zoomRatio * (mx - m_GridPanOffset.x - (avail.x - baseCellSize * oldZoom *
					sizeX) * 0.5f)
				- (avail.x - baseCellSize * m_GridZoom * sizeX) * 0.5f;
				m_GridPanOffset.y = my - zoomRatio * (my - m_GridPanOffset.y - (avail.y - baseCellSize * oldZoom *
					sizeZ) * 0.5f)
				...
				if (filledIdx >= 0) {
					...
					if (cellSize >= 12.0f) {
						...
						drawList->AddRectFilled(pMin, iMax,
						                        IM_COL32(static_cast<int>(entry.Color.x * 255),
						                                 static_cast<int>(entry.Color.y * 255),
						                                 static_cast<int>(entry.Color.z * 255), 255));
					}
				}
				else {
					// Empty cell
		...
		if (cellSize >= 8.0f) {
			for (int x = 0; x <= sizeX; x++) {
				...
				drawList->AddLine(ImVec2(px, gridOrigin.y), ImVec2(px, gridOrigin.y + gridH),
				                  IM_COL32(60, 60, 70, 180));
			}
			for (int z = 0; z <= sizeZ; z++) {
				float pz = gridOrigin.y + z * cellSize;
				drawList->AddLine(ImVec2(gridOrigin.x, pz), ImVec2(gridOrigin.x + gridW, pz),
				                  IM_COL32(60, 60, 70, 180));
			}
		}
		...
		if (wasClicked && m_HoveredCell.x >= 0 && m_HoveredCell.y >= 0) {
			...
			if (filledIdx >= 0 && filledIdx < static_cast<int32_t>(m_Vmesh->PaletteIndices.size())) {
				...
				if (currentPalIdx != static_cast<uint8_t>(m_SelectedBrushIndex)) {
					...
					m_Vtex->PaletteIndexOverrides[static_cast<uint32_t>(filledIdx)] = static_cast<uint8_t>(
						m_SelectedBrushIndex);
					m_MaterialMapDirty = true;
				}
			}
		}
	...
	void VoxelTextureEditorPanel::RebuildColorGroups()
	{
		...
		uint32_t usedCount = m_Vmesh->Palette.GetUsedCount();
		constexpr float threshold = 0.40f; // Euclidean distance in [0,1] RGB space
		...
		for (uint8_t i = 0; i < usedCount; i++) {
			...
			for (int g = 0; g < static_cast<int>(m_ColorGroups.size()); g++) {
				float dist = length(m_ColorGroups[g].AverageColor - color);
				if (dist < bestDist) {
					bestDist = dist;
					bestGroup = g;
				}
	...
	void VoxelTextureEditorPanel::OnImGuiRender()
	{
		...
		if (ImGui::Begin("Voxel Texture Editor", &m_Visible)) {
			...
			if (ImGui::BeginPopup("GenerateVtexPopup")) {
				...
				static auto pickerVmeshUUID = UUID(0);
					...
					if (ImGui::Button("Generate .vtex")) {
						...
						if (vmesh) {
							...
							if (newVtex) {
								...
								if (vmeta) {
									std::filesystem::path vmeshFullPath = AssetManager::GetAssetDirectory() / vmeta->
									FilePath;
	...
	void VoxelTextureEditorPanel::RenderPaletteEditor()
	{
		...
		if (ImGui::Button("Save")) {
			...
			else { m_Vtex->Save(m_Vtex->GetPath()); }
		}
		...
		else { ImGui::TextDisabled("No brush selected (click a color below)"); }
		...
		if (m_ShowGroupedView) {
			// ---- Grouped view ----
			ImGui::Text("Color Groups (%u):", static_cast<uint32_t>(m_ColorGroups.size()));
			...
			for (int g = 0; g < static_cast<int>(m_ColorGroups.size()); g++) {
				...
				for (uint8_t idx : group.EntryIndices) {
					if (static_cast<int>(idx) == m_SelectedBrushIndex) {
						groupHasBrush = true;
						break;
					}
				}
				...
				if (ImGui::ColorButton(
					"##grpcol", ImVec4(group.AverageColor.r, group.AverageColor.g, group.AverageColor.b, 1.0f),
					ImGuiColorEditFlags_NoTooltip, ImVec2(20, 20))) { m_SelectedBrushIndex = group.EntryIndices[0]; }
				...
				char label[64];
				snprintf(label, sizeof(label), "(%u)%s", static_cast<uint32_t>(group.EntryIndices.size()),
				         groupHasBrush ? " [B]" : "");
				...
				if (group.Expanded) {
					...
					for (uint8_t idx : group.EntryIndices) {
						...
						bool isBrush = (static_cast<int>(idx) == m_SelectedBrushIndex);
			...
			for (uint8_t i = 0; i < usedCount; i++) {
				...
				const auto& entry = m_Vmesh->Palette.GetEntry(i);
				bool isBrush = (static_cast<int>(i) == m_SelectedBrushIndex);
	...
	void VoxelTextureEditorPanel::RemoveBrush(uint8_t paletteIndex)
	{
		...
		if (m_SelectedBrushIndex == static_cast<int>(paletteIndex))
			m_SelectedBrushIndex = -1;
