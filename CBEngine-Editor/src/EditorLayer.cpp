...
namespace CB
{
	...
	void EditorLayer::EndDockspace() { ImGui::End(); }

	void EditorLayer::NewScene() { SceneManager::NewScene(); }

	void EditorLayer::OpenScene()
	{
		String filepath = FileDialogs::OpenFile("CBEngine Scene (*.scene)\0*.scene\0All Files (*.*)\0*.*\0");
		if (!filepath.empty()) { SceneManager::LoadScene(filepath); }
	}
	...
	void EditorLayer::SaveSceneAs()
	{
		...
		if (!filepath.empty()) { SceneManager::SaveSceneAs(filepath); }
	}
